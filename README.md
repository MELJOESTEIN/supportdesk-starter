...
├── backend/                      ← Spring Boot 4.1 — JPA, pagination, DTO, ProblemDetail
├── frontend/                     ← Angular 22 — les onze écrans de la maquette
└── verif/                        ← éprouver l'API à la main (REST Client)
```

> **Branche `depart-j2`.** Elle porte le résultat du jour 1 : une chaîne complète qui tourne,
> du navigateur à PostgreSQL. **Elle ne contient aucune sécurité** — ni validation de jeton, ni
> rôles, ni gardes de route. C'est ce que tu construis aujourd'hui.
>
> Aujourd'hui, `GET /api/tickets` répond `200` **sans aucun jeton**. En fin de journée, il devra
> répondre `401`. Entre les deux, il y a le jour 2.
...
L'application est sur **http://localhost:4200**. Le profil `dev` allume les logs SQL — tu en
auras besoin.

### Le mode tuteur

Tu apprends Spring Boot en l'écrivant, pas en le lisant. Dans ta session Claude Code :

```
/tuteur-spring-boot
```

L'agent bascule alors en formateur : il ne code plus à ta place, il te fait coder. Détail dans
