 ...
 *
 * <p>Depuis l'ajout du resource server, ce slice charge l'autoconfiguration OAuth2, qui
 * exige un {@code issuer-uri} : sans lui, le contexte ne se construit plus et les cinq
 * tests tombent d'un coup. Les propriétés sont donc déclarées ici — un slice n'hérite pas
 * du profil des tests d'intégration.
 */
@WebMvcTest(TicketController.class)
@TestPropertySource(properties = {
		"spring.security.oauth2.resourceserver.jwt.issuer-uri=http://localhost:8081/realms/supportdesk",
		"supportdesk.audience=supportdesk-api",
		"supportdesk.origines-autorisees=http://localhost:4200" })
class TicketControllerTests {
	...
	void liste_respecteLeContrat() {
		given(this.service.lister(any(), any(), any())).willReturn(unePage());

		var corps = assertThat(this.mvc.get().uri("/api/tickets").with(Jetons.alice())).hasStatusOk().bodyJson();
	...
	void detail_inexistant_renvoieProblemDetail() {
		willThrow(new TicketIntrouvableException(999L)).given(this.service).detail(eq(999L), any());

		var corps = assertThat(this.mvc.get().uri("/api/tickets/999").with(Jetons.bob()))
				.hasStatus(404)
	...
	void creation_sujetVide_renvoieProblemDetail() {
		assertThat(this.mvc.post().uri("/api/tickets").with(Jetons.alice())
	...
	void creation_statutImpose_estIgnore() {
		given(this.service.creer(any(), any())).willReturn(unDetail());
	...
	private static TicketDetail unDetail() {
		return new TicketDetail(1L, "TCK-4900", "Test", StatutTicket.OUVERT, PrioriteTicket.NORMALE,
				"CLI-0001", null, null, null, Instant.parse("2026-08-30T08:00:00Z"),
